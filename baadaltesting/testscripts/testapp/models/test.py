        ...
        for j in xrange(0,len(root[i])):
            ...
            if page_present!="None":
                ...
                if vm_status==0:
                    ...
                    logger.debug("Your VM has not created.Please Check it!!!Its in pending task table!!!Either Scheduler is not working or Host is down!!!")
                if vm_status==2:
...
def get_task_start_time(driver,xml_sub_child,xml_child,vm_name,operation_name):
    ...
    if qury_result!=():
        ...
        if (query_result!="None") | (query_result!=()):
            ...
            logger.debug(xml_child.get("value")  + "Error")
        baadal_db.commit()
        ...
        if ((str(total)==str(count))) :
            for col in field:
                ...
                if (field_text!=""):
                    ...
                    | (xml_parent.get("name")==status_list[22] ))) :
                    ...
                    elif (col_count%int(length)==2) & ( (xml_parent.get("name")==status_list[17]) | (xml_parent.get("name")==status_list[21]) | (xml_parent.get("name")==status_list[14]) | (xml_parent.get("name")==status_list[15])) | ((col_count%int(length)==5) &  (xml_parent.get("name")==status_list[16])) | ((col_count%int(length)==6) &  ((xml_parent.get("name")==status_list[11])  | (xml_parent.get("name")==status_list[22]) | (xml_parent.get("name")==status_list[6]))) | ((col_count%int(length)==3) & ( (xml_parent.get("name")==status_list[4]) | (xml_parent.get("name")==status_list[7]) | (xml_parent.get("name")==status_list[5]))) | ((col_count%int(length)==4) &  (xml_parent.get("name")==status_list[12])):
                        status=query_result[row_count][col_count]
