...
#################################################################################################################
#                                       The main test function  for unit testing & integeration testing                                    #
#################################################################################################################
...
def test_script(test_case_no):
    ...
    if root[num-1].get("id")==test_case_no:
        ...
        for j in xrange(0,len(root[i])):
            ...
            driver = webdriver.Firefox()#create an instance of firefox
            ...
            for k in xrange(0,len(root[i][j])):
                if vm_status:
                    ...
                    if field_type=="input": #checking for text fields
                        ...
                        isWait(driver,xml_parent,xml_child,xml_sub_child)
                    
                    elif field_type=="check_data":
                        isCheckdata(driver,xml_parent,xml_child,xml_sub_child,vm_name)#checking for data in host table
                 
                    elif field_type=="task_table":
                         operation_name=xml_sub_child.text
                         vm_status1=check_vm_task(driver,xml_sub_child,xml_child,vm_name,operation_name)#checking for data in task table
                
                    elif field_type=="attach_disk":
                        operation_name=xml_sub_child.text
                        attack_disk(driver,xml_sub_child,xml_child,vm_name,operation_name)#checking for data in task table for attach_disk
...
'''def check_stat_on_host():
    ...
    query_result=execute_query("select vm_name,vm_data.status,host_id from vm_data join host where host.id=vm_data.host_id and host_ip='10.208.21.70'").fetchall()
    datas=str(query_result)
    lists=datas.split("), (")
    col_count=len(lists)
    print lists
    print col_count
        ...
        for id in conn.listDomainsID():
            ...
            if newstr==dom.name():
                ...
                if (((datass[1]==" 5L") & (infos[0]==1)) | ((datass[1]==" 6L") & (infos[0]==3))):
                    print "VM is on expected host" '''                                           
 
#################################################################################################################
#                             The main test function for stress testing
                                           #
#################################################################################################################
...
def stress_test_script():
    for m in range(1,5):
        ...
        thread.start_new_thread( test_script,(test_no,) )
        thread.start_new_thread( test_script,(test_no,) )
        thread.start_new_thread( test_script,(test_no,) )
        thread.start_new_thread( test_script,(test_no,) )
        thread.start_new_thread( test_script,(test_no,) )
...
op_list={'revert_to_snapshot':'Revert Snapshot','delete_snapshot':'Delete Snapshot','snapshot':'Snapshot VM','pause_machine':'Suspend VM','Delete':'Delete VM','shutdown_machine':'Stop VM','destroy_machine':'Destroy VM','start_machine':'Start VM','user_details':'Add User','attach_extra_disk':"Attach Disk",'clone_vm':'Clone ','delete_user_vm':'Delete User','adjrunlevel':'Adjust Run Level','edit_vm_config':'Edit VM Config','resume_machine':'Resume VM','migrate_vm':'Migrate VM'}
...
#printing result corresponding to snapshot
...
#checking graph when vm is in shutdown mode
...
def isSubmit( driver, xml_parent, xml_child, xml_sub_child):
	...
	if isElementPresent(driver,xml_child,xpath):
...
def isTable(driver,xml_parent,xml_child,xml_sub_child):
    status_list={0:"Error",1:"failed_tasks",2:"TRY AGAIN | IGNORE",3:"my_pending_vm",4:"admin_pending_attach_disk",5:"pending_install_vm",6:"pending_clone_vm",7:"pending_attach_disk",8:"pending_edit_conf",9:"host_and_vm",10:"Configure_host",11:"admin_pending_clone_vm",12:"admin_pending_install_vm" ,13:'Configure_security'}
    table_path=xml_sub_child.text
    if isTablePresent(driver,xml_child,table_path):
        cur=execute_query( xml_parent.get("query3"))
        ...
        if (str(total)==str(count)):
            for col in field:
                ...
                if (field_text!=""):
                    ...
                    elif (col_count%int(length)==2) & ((xml_parent.get("name")==status_list[5]) | (xml_parent.get("name")==status_list[7]))| (col_count%int(length)==5) & (xml_parent.get("name")==status_list[6]) | (col_count%int(length)==3) & (xml_parent.get("name")==status_list[4]) | ((col_count%int(length)==6) & (xml_parent.get("name")==status_list[11])) | ((col_count%int(length)==4) & (xml_parent.get("name")==status_list[12])):
                        result=str(query_result[row_count][col_count]) + " CPU"
                        print_result(field_text,result,xml_child)
                    
                    elif (col_count%int(length)==3) &((xml_parent.get("name")==status_list[5]) | (xml_parent.get("name")==status_list[6]) | (xml_parent.get("name")==status_list[7])) | (col_count%int(length)==4) & ((xml_parent.get("name")==status_list[4]) | (xml_parent.get("name")==status_list[11])) | ((col_count%int(length)==5) & (xml_parent.get("name")==status_list[12])):
                        ram=query_result[row_count][col_count]
                        result=check_vm_ram(ram)
                        print_result(field_text,result,xml_child) 
                    
                    elif (col_count%int(length)==4) & ((xml_parent.get("name")==status_list[5]) |(xml_parent.get("name")==status_list[6])) | ((col_count%int(length)==5) & (xml_parent.get("name")==status_list[11])) | ((col_count%int(length)==6) & (xml_parent.get("name")==status_list[12])):
                        extra_disk=query_result[row_count][col_count]
                        result=check_extra_disk(extra_disk)
                        print_result(field_text,result,xml_child)   
                    
                    elif (col_count%int(length)==5) & (xml_parent.get("name")==status_list[5]) | ((col_count%int(length)==7) & (xml_parent.get("name")==status_list[12])):
                        if (xml_parent.get("name")==status_list[5]):
                            vm_name=query_result[row_count][1]
                        if (xml_parent.get("name")==status_list[12]):
                            vm_name=query_result[row_count][4]
                        result=check_port_enabled(vm_name)
                        print_result(field_text,result,xml_child) 
                    
                    elif (col_count%int(length)==6) & (xml_parent.get("name")==status_list[5])| ((col_count%int(length)==8) & (xml_parent.get("name")==status_list[12])):
                        security_domain=query_result[row_count][col_count]
                        result=check_security_domain(security_domain)
                        print_result(field_text,result,xml_child)   
                    
                    elif ((col_count%int(length)==7) & ((xml_parent.get("name")==status_list[5]) | (xml_parent.get("name")==status_list[7]))) |  (col_count%int(length)==6) & (xml_parent.get("name")==status_list[6]):
                        status=query_result[row_count][col_count]
                        result=org_task_status(status)
                        print_result(field_text,result,xml_child) 
   
                    elif ((col_count%int(length)==4) | (col_count%int(length)==5) | (col_count%int(length)==6 )) & (xml_parent.get("name")==status_list[7]) | ((col_count%int(length)==5)| (col_count%int(length)==6) | (col_count%int(length)==7)) & (xml_parent.get("name")==status_list[4]):
                        mem=query_result[row_count][col_count]
                        result=check_mem(mem)
                        print_result(field_text,result,xml_child)  
                 
                    elif (((col_count%int(length)==8)| (col_count%int(length)==9)) & (xml_parent.get("name")==status_list[4])) | (((col_count%int(length)==7) | (col_count%int(length)==8)) & (xml_parent.get("name")==status_list[11])) | (((col_count%int(length)==9)| (col_count%int(length)==10)) & (xml_parent.get("name")==status_list[12])):
