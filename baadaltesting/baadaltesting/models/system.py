...
def sys_test_script(test_case_no,host_ip,my_logger):
    ...
    my_logger.debug("num is : "+str(num))
    vm_status=1
    public_ip =""
    my_logger.debug(root[num-1].get("id"))
    if root[num-1].get("id")==str(test_case_no):       
        i=num-1
        vm_name=""  
        for j in xrange(0,len(root[i])):
            temp_value=1
            my_logger.debug("inside for loop")       	
            ...
            if page_present!="None":
            	...
            	for k in xrange(0,len(root[i][j])):
                    if vm_status:
                        ...
                        my_logger.debug("task type is : " + str(field_type))
				...
				public_ip=create_file(driver,xml_child,xml_sub_child,baadal_db,my_logger)

                        elif field_type=="shutdown_vm":
				shutdown_vm_test(driver,xml_child,xml_sub_child,baadal_db,my_logger)
                        	...
                        	attach_disk(driver,xml_child,xml_sub_child,baadal_db,my_logger)
                        	...
                        	clone_vm(driver,xml_child,xml_sub_child,baadal_db,my_logger)
                            ...
                            main_host_check(driver,xml_child,xml_sub_child,my_logger)
                             ...
                             edit_vm_conf(driver,xml_sub_child,xml_child,data,my_logger) 
		             ...
		             data=vm_list_my_vm(driver,xml_child,xml_sub_child,my_logger) 
                             ...
                             create_vm_for_image_testing(driver,xml_child,xml_sub_child,host_ip,my_logger)
...
def other_operation_on_vms(driver,xml_sub_child,xml_child,vm_name,vm_id,my_logger):
	...
	total=check_pendingtasks_table(driver,xml_sub_child,xml_child,vm_name,operation_name,vm_user_list,my_logger)
        my_logger.debug("total"+str(total))
	driver.find_element_by_partial_link_text("All VMs").click()'''
	...
	if total==[]:
            my_logger.debug("inside total")
            my_logger.debug(xml_sub_child)
...
def click_on_operations(driver,xml_sub_child,xml_child,vm_name,vm_id,op_name,baadal_db,my_logger):   
    my_logger.debug("inside click on operation")
    my_logger.debug("click on op op_name is " + str(op_name))  
    if op_name=="Delete":
        limit=delete_vm(driver,xml_sub_child,xml_child,vm_name,vm_id,my_logger)
    elif op_name=="attach_extra_disk":
	limit=operation_on_attach_disk(driver,xml_sub_child,xml_child,vm_name,vm_id,my_logger)
	...
	limit=op_clone_vm(driver,xml_sub_child,xml_child,vm_name,vm_id,my_logger)
    elif op_name=="start_machine":
	print "start vm"
	limit=op_start_vm(driver,xml_sub_child,xml_child,vm_name,vm_id,my_logger)
        my_logger.debug("return back after performing start vm")
    else:
	my_logger.debug("other opn")
        limit=other_operation_on_vms(driver,xml_sub_child,xml_child,vm_name,vm_id,my_logger)
        my_logger.debug("return back to click on operation")
    return limit
####################################################################################################################
...
def isSelect2(driver, xml_sub_child,path,my_logger):
	...
	if xml_sub_child.get("select_name")=="configuration":
	    option_value=1
	return option_value
def create_template_from_vm_image(driver,xml_child,xml_sub_child,my_logger):
    ...
    my_logger.debug("after vm_all_info")
    data.append(data1[0][0:])
    data.append(data1[2][0:])
    my_logger.debug("After appending data from vm_all_info to main data list : "+str(data)) 
    if xml_sub_child.get("create_template_status")=="1":
        op_stop_vm(driver,xml_sub_child,xml_child,vm_name,vm_id,my_logger)
...
def request_vm(driver,xml_child,xml_sub_child,data1,my_logger):
    if data1!=['','','','','','','']:
        ...
        my_logger.debug("Disk space of  VM is :"+str(disk_space))
        template_name = str(data[2][0:]) +" "+str(data[3][0:])+ " "+str(data[4][0:])+" "+str(data[5][0:])+" "+template_space+"("+str(data[0][0:])+")"
        my_logger.debug("template_name :"+str(template_name))
...
def list_details_my_template(driver,xml_child,xml_sub_child,vm_name,my_logger):
    my_logger.debug("inside list_details_my_template")
    data1=[]
    ...
    if isTablePresent(driver,xml_child,path_col,my_logger):
        ...
        for hdata in header_field:
            ...
            count+=1
        col_count=count
        my_logger.debug( "after checking header count in list_details my templates")
        field=driver.find_elements_by_xpath(path_col)
        my_logger.debug(field)
        os_details=data[5][0:].split(' ')
        for data in field:
            if c_count%col_count==template_name_no:
                my_logger.debug("vm_name :"+str(vm_name)+"_template")
            ...
            if c_count%col_count==os_name_no:
                my_logger.debug("os_name :"+str(os_details[0][0:]))
                if data.text==data[0][0:]:
                    os_name=data.text
                else :
                    continue
            if c_count%col_count==os_version_no:
                my_logger.debug("os_version :"+str(os_details[1][0:]))
            ...
            if c_count%col_count==os_type_no:
                my_logger.debug("os_type :"+str(os_details[2][0:]))
                if data.text==os_details[2][0:]:
                    os_type=data.text
                else :
                    continue
            if c_count%col_count==architecture_no:
                my_logger.debug("architecture :"+str(os_details[3][0:]))
            ...
            c_count+=1
    if c_count==0:
        my_logger.debug("No template is found with the  vm_name")
        return None
...
def create_vm_for_image_testing(driver,xml_child,xml_sub_child,ssh_ip,my_logger):
    data =vm_list_my_vm(driver,xml_child,xml_sub_child,my_logger) #get public ip of vm
    my_logger.debug(data)
    my_logger.debug("u r in  create_vm_for_image_testing")
    if data!=['','','','']:
        vm_id=data[0][3:]
        vm_name=data[1][0:]
        public_ip=data[2][0:]
        public_ip=str(public_ip)
    my_logger.debug("vm id is : " + str(vm_id))
    my_logger.debug("data values :"+str(data))
    driver.refresh()
    my_logger.debug("u r in my function")
    data1=vm_all_info(driver,xml_child,xml_sub_child,data,my_logger)
    my_logger.debug("after vm_all_info")
    for val in data1:
        data.append(val)
    ...
    my_logger.debug("data returned from find_vm_id :"+str(data2))
...
def vm_all_info(driver,xml_child,xml_sub_child,data,my_logger):
    my_logger.debug("Inside vm_all_info function  ")
    vm_id=data[0][3:]
    print vm_id
    vm_name=data[1][0:]
    click_on_setting(driver,xml_sub_child,xml_child,vm_name,vm_id,my_logger)
    ...
    countc=0
    if isTablePresent(driver,xml_child,path_col,my_logger):
        my_logger.debug("return back to vm_all_info after checking table present")
        c_count=0
        r_count=0
        select_row=0
        select=0
        ...
        for hdata in header_field:
            ...
            count+=1
        col_count=count
        my_logger.debug( "after checking header count")
        ...
        for data in field:
            ...
            c_count+=1
    my_logger.debug( "security domain:"+str(security_domain_name))
    ...
    driver.find_element_by_xpath(path).click()
    driver.refresh()
    my_logger.debug("stop_vm")
    my_logger.debug("return value from  vm_all_info : " + str(data2))
    ''' 
    return data2
...
def create_vm_for_image(driver,xml_child,xml_sub_child,data,ssh_ip,my_logger):
    my_logger.debug("inside create_vm_for_image ")
    if data!=['', '','','','','','','','','','','','']:
        print data
        vm_id=data[0][3:]
        vm_name=str(data[1])
        ...
        if xml_sub_child.get("public_ip_flag") == "1":
            ...
            public_ip=""
        my_logger.debug("Public IP assigned to vm is :"+str(public_ip))
        if xml_sub_child.get("private_ip_flag") == "1":
            private_ip=data[8][0:]
        else:
            private_ip=""
        my_logger.debug("Private IP assigned to vm is :"+str(private_ip))
        cpuinfo=str(data[9][0:]).replace(" CPU","")
        my_logger.debug("Total no of cpu in VM :"+str(cpuinfo))
        host_ip=data[10][0:]
        my_logger.debug("Host IP of the VM is :"+str(host_ip))
        if '+' in data[4][0:]:
            add_extra_drive =1    
        disk_space=str(data[4][0])+str(data[4][1])+str(data[4][3])+str(data[4][4])
        my_logger.debug("Disk space of  VM is :"+str(disk_space))
        ram=str(data[5][0:]).replace(" MB","")
        my_logger.debug("RAM for vm is :"+str(ram))
        operating_sys_name=str(data[6][0:])
        my_logger.debug("VM operating system name :"+str(operating_sys_name))
        ...
        my_logger.debug("VM image name :"+str(vm_image_name))
        ...
        my_logger.debug("Template id :"+str(template_id))
        security_domain=add + security_domain + add
        my_logger.debug("security domain for vm :"+str(security_domain))
        my_logger.debug("attaching extra disk")
        if xml_sub_child.get("disk_status")== "1" and add_extra_drive== 1:
            ...
            my_logger.debug("no extra disk is attached Please add extra disk to vm and try again ")
        else:
            attach_disk_name=""
            my_logger.debug("attaching extra disk when empty")
    my_logger.debug("after attaching extra disk")    
	#vm_requester="badalUFA"
    ...
    my_logger.debug("before scroll")
    isScroll(driver, xml_sub_child,my_logger)
    my_logger.debug("after scroll")
    ...
    my_logger.debug("//table/tbody/tr/td/input[@name='private_ip']/option[text()="+private_ip+"]")
...
def undefine_vm(driver,xml_child,xml_sub_child,ssh_ip,host_ip,vm_image_name,my_logger):
    my_logger.debug("inside undefine vm")
    ...
    my_logger.debug("output :"+str(var1))
...
def click_on_sanity_check_operation(driver,xml_child,xml_sub_child,vm_name,my_logger):
    my_logger.debug("inside click on sanity check operation")
    isScroll(driver, xml_sub_child,my_logger)
    ...
    my_logger.debug("_________________________")
    if isTablePresent(driver,xml_child,path_col,my_logger):
        ...
        col_count=count
        field=driver.find_elements_by_xpath(path_col)
        my_logger.debug("before data in  datafield")
        for data in field:
		...
		if select_vm and click_flag:
            ...
            c_count+=1
        my_logger.debug("end of the click on sanity ")
...
def operation_on_edit_vm_conf(driver,xml_sub_child,xml_child,vm_name,vm_id,my_logger):
    ...
    field_text=message_flash(driver,xml_sub_child,path,my_logger)
...
def operation_on_attach_disk(driver,xml_sub_child,xml_child,vm_name,vm_id,my_logger):
    ...
    my_logger.debug("op : " + str(operation_name))    
    xpath=task_path_vm(xml_sub_child,op_name,my_logger)    
    my_logger.debug("xpath is : " + str(xpath))
    path="//*[@title='" + str(xpath) + "']" 
    my_logger.debug("path is : " + str(path))
    if isElementPresent(driver,xml_child,path,my_logger):
    	driver.find_element_by_xpath(path).click()
    	add_value(driver,xml_sub_child,xml_child,vm_name,vm_id,my_logger)
        check_attach_disk(driver,xml_sub_child,xml_child,vm_name,my_logger)
...
def op_clone_vm(driver,xml_sub_child,xml_child,vm_name,vm_id,my_logger):
    my_logger.debug("inside op_clone_vm")  
    op_stop_vm(driver,xml_sub_child,xml_child,vm_name,vm_id,my_logger)
    driver.refresh()
    print "reboot"
    op_name=xml_sub_child.get("op")
...
def op_stop_vm(driver,xml_sub_child,xml_child,vm_name,vm_id,my_logger):
    my_logger.debug("inside stop vm")
    op_name=xml_sub_child.get("op1")
    my_logger.debug("in stop vm op_name : "+ str(op_name))
    operation_name=op_list[op_name]
    my_logger.debug("in stop vm op : " +str(operation_name))    
    xpath=task_path_vm(xml_sub_child,op_name,my_logger) 
    my_logger.debug("xpath is : " + str(xpath))
    path="//*[@title='" + str(xpath) + "']"
...
def op_start_vm(driver,xml_sub_child,xml_child,vm_name,vm_id,my_logger):
...
def operation_on_migrate_vm(driver,xml_sub_child,xml_child,vm_name,vm_id,my_logger):
    my_logger.debug("inside operation_on_migrate_vm")
    op_name=xml_sub_child.get("op")
    limit=0
    op_name=xml_sub_child.get("op")
    operation_name=op_list[op_name]
...
def check_vm_in_pending_request_table(driver,xml_sub_child,xml_child,vm_name,old_field,my_logger):
    my_logger.debug("check_vm_in_pending_request_table")  
        ...
        for data in field1:            
            if col_count%total_col==vm_no:		
                if vm_name in data.text:
                    my_logger.debug("vm_name" + str(data.text))
                    s_row=1	      
            if col_count%total_col==user_no:
                my_logger.debug(xml_child[0].text)
                usernm=usrnm_list[xml_child[0].text]        
                if str(data.text)==usernm:
                    my_logger.debug(data.text)
                    select_row=1   		   
            if col_count%total_col==(total_col-1) :
                my_logger.debug("value of select_row : " + str(select_row))
                my_logger.debug("value of s_row : " + str(s_row))
                if select_row :
                    if s_row:
			data1=data.find_element_by_tag_name("a")
                        value=data1.get_attribute("id")
                        my_logger.debug("id" + str(value))				
                        vm_id=value[7:]
                        my_logger.debug("vm_id"+str(vm_id))
                        break
            col_count+=1    
    return vm_id
...
def check_clone_vm(driver,xml_sub_child,xml_child,vm_name,my_logger):
    ...
    print vm_id
    if xml_sub_child.get("action")=="approve_request":   
	    my_logger.debug("approve_request") 
            print "//*[@href='/baadal/admin/approve_request/"+ str(vm_id) +"']"
            driver.find_element_by_xpath("//*[@href='/baadal/admin/approve_request/"+ str(vm_id) +"']").click() 
            time.sleep(120)
            my_logger.debug("approve_clone_request")      
    else:
...
def vm_mode_operation(xml_child,xml_sub_child,driver,vm_name,vm_id,baadal_db,my_logger):
    op_name=xml_sub_child.get("op") 
    my_logger.debug("vm _mode op_name is : " + str(op_name))   
    operation_name=op_list[op_name]
    my_logger.debug("vm _mode operation_name is : " + str(operation_name)) 
    #task_value=perform_task_operation(driver,xml_sub_child,xml_child,vm_name,operation_name,my_logger)
    driver.find_element_by_partial_link_text("All VMs").click()
    click_on_setting(driver,xml_sub_child,xml_child,vm_name,vm_id,my_logger)
    my_logger.debug("return back to vm_mode_operation")
    limit=click_on_operations(driver,xml_sub_child,xml_child,vm_name,vm_id,op_name,baadal_db,my_logger)	
    if limit:
       time_to_check_in_tasktable(my_logger)
       task_n_value=perform_task_operation(driver,xml_sub_child,xml_child,vm_name,operation_name,my_logger)
       compare_task_table(driver,xml_sub_child,xml_child,vm_name,operation_name,task_value,task_n_value,my_logger)
    return
   	   

def vm_list_my_vm(driver,xml_child,xml_sub_child,my_logger):
    my_logger.debug("inside vm_list_my_vm")
    data2=[]
	...
	select_status=0
		...
		public_ip_no=count
            count+=1
        col_count=count
        my_logger.debug("col_count is : " + str(col_count))
        my_logger.debug("vm_no is : " + str(vm_name_no))
        my_logger.debug("user no : " + str(user_name_no))
        my_logger.debug("status is : " + str(status_no))
        my_logger.debug("public ip is : " + str(public_ip_no))
        field=driver.find_elements_by_xpath(path_col)
        for data in field:
            my_logger.debug("c_count is : " + str(c_count))
            if c_count%col_count==vm_name_no:
                vm_name=data.text
            if c_count%col_count==user_name_no:
                for username in username_list:
                    ...
                    my_logger.debug(data.text)
		    if username==data.text:
                       my_logger.debug("inside if user")
                       user_name=data.text
                       select_row=1
                       break
                my_logger.debug("select row is : " + str(select_row))
            if c_count%col_count==status_no:
                my_logger.debug("inside status c_count is : " + str(c_count))
                status=data.text
		if status=="Running":
                   select_status=1
	    if c_count%col_count==public_ip_no:
                my_logger.debug("inside public ip c_count is " + str(c_count))
                public_ip=data.text
		if public_ip == "Not Assigned":
                   select=0
		else:
	           select=1
            my_logger.debug("value is : " + str(c_count%col_count))
            my_logger.debug("value of col_count-1 is : " + str(col_count-1))
            if (c_count%col_count == col_count-1 ):
                my_logger.debug("select row : " + str(select_row) + "select : " + str(select) + "select_status : " + str(select_status))
	        if select_row and select and select_status:
                    my_logger.debug("inside if for select id")
                    my_logger.debug(type(status))	
                    my_logger.debug(xml_sub_child.get("status"))    
                    if (str(status)==xml_sub_child.get("status")):
                        my_logger.debug("inside if for status")
...
####################### Migration of VM ############################                          
def create_file(driver,xml_child,xml_sub_child,baadal_db,my_logger):
    data = vm_list_my_vm(driver,xml_child,xml_sub_child,my_logger)
    my_logger.debug("return back to create file")    
    if data!=['', '','','']:
        vm_id=data[0][3:]
        vm_name=data[1][0:]
        public_ip=data[2][0:]
...
def shutdown_vm_test(driver,xml_child,xml_sub_child,baadal_db,my_logger):
    data = vm_list_my_vm(driver,xml_child,xml_sub_child,my_logger) #get public ip of vm
    my_logger.debug("return back to shutdown")
    my_logger.debug(data)
    if data!=['', '','','']:
        vm_id=data[0][3:]
        vm_name=data[1][0:]
        ...
        public_ip=str(public_ip)
        my_logger.debug(public_ip)
        vm_mode_operation(xml_child,xml_sub_child,driver,vm_name,vm_id,baadal_db,my_logger) #click seeting button and shutdown vm
        my_logger.debug("return back after vm_mode_operation")
        ssh = paramiko.SSHClient()
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
...
def attach_disk(driver,xml_child,xml_sub_child,baadal_db,my_logger):
    my_logger.debug("inside attach disk function")
    data = vm_list_my_vm(driver,xml_child,xml_sub_child,my_logger) #get public ip of vm
    my_logger.debug("back to attach disk")
    my_logger.debug(data)
    if data!=['', '','','']:
        vm_id=data[0][3:]
        vm_name=data[1][0:]
...
def clone_vm_list(driver,xml_child,xml_sub_child,vm_name,my_logger):
    my_logger.debug("inside clone vm list")
    data2=[]
    path_col="//table[@id='myvms']/tbody/tr/td"
    path_row="//table[@id='myvms']/tbody/tr"
    path_header="//table[@id='myvms']/thead/tr/th"
    ...
    if isTablePresent(driver,xml_child,path_col,my_logger):
        my_logger.debug("inside table checking")
        countc=0
        c_count=0
        r_count=0
        select_row=0
	...
	select_vm=0
        ...
        for data in field:
            if c_count%col_count==vm_name_no:
                vm_name1=data.text
                my_logger.debug("vm name is : " + str(vm_name1))
                my_logger.debug("new vm name is : " + str(new_vmname))
	    ...
	    if c_count%col_count==private_ip_no:
                private_ip=data.text
            if (c_count%col_count==col_count-1):
                my_logger.debug("select_vm : " + str(select_vm))
                my_logger.debug("select_row : " + str(select_row))
...
def clone_vm(driver,xml_child,xml_sub_child,baadal_db,my_logger):
    my_logger.debug("inside clone vm")
    data = vm_list_my_vm(driver,xml_child,xml_sub_child,my_logger) #get public ip of vm
    if data!=['', '','','']:
        vm_id=data[0][3:]
        vm_name=data[1][0:]
    ...
    chk=execute_remote_cmd(public_ip,"root","grep MemTotal /proc/meminfo", "baadal",my_logger, ret_list = False)    
    vm_mode_operation(xml_child,xml_sub_child,driver,vm_name,vm_id,baadal_db,my_logger) 
    driver.find_element_by_partial_link_text("All VMs").click() 
    click_on_setting(driver,xml_sub_child,xml_child,vm_name,vm_id,my_logger)
    op_name=xml_sub_child.get("op2")
    ...
    limit=click_on_operations(driver,xml_sub_child,xml_child,vm_name,vm_id,op_name,baadal_db,my_logger)
    my_logger.debug("return back to clone vm")
    time.sleep(20) 
    op_name=xml_sub_child.get("op2")
    my_logger.debug("vm name is : " + str(vm_name))
    ...
    my_logger.debug("vm_id is : " + str(vm_id2))
    my_logger.debug("private ip is : " + str(private_ip))
    driver.find_element_by_partial_link_text("All VMs").click()
    click_on_setting(driver,xml_sub_child,xml_child,vm_name2,vm_id2,my_logger)
    my_logger.debug("return back click on setting")
    limit=click_on_operations(driver,xml_sub_child,xml_child,vm_name2,vm_id2,op_name,baadal_db,my_logger)
    time.sleep(120)
    my_logger.debug("return back to cloning vm") 
    try:
        ssh = paramiko.SSHClient()
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ssh.connect(public_ip,username="root", password="baadal")
    	my_logger.debug("connect to clone vm")
    	stdin, stdout,stderr=ssh.exec_command("ssh -X root@"+private_ip+" grep MemTotal /proc/meminfo")
    	new_chk=stdout.readlines()
    	my_logger.debug(new_chk)
    	print new_chk
    	stdin, stdout,stderr=ssh.exec_command("ssh -X root@"+private_ip+" fdisk -l | egrep 'Disk.*bytes'| awk '{ sum +=$3;} END {print sum}'")
    	new_check=stdout.readlines()
    	my_logger.debug(new_check)
    except:
...
def shutdown(driver,my_logger):
        my_logger.debug("inside shutdown function")
...
def check_baadal_shutdown1(public_ip,driver,my_logger):
    ...
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    try:
         check=ssh.connect(public_ip,username="root", password="duolc")    
         return 1
    except:
    ...
    if isTablePresent(driver,xml_child,path_col,my_logger):
        ...
        for data in field:
            ...
            if (c_count%col_count==col_count-1):
                row_count+=1
                if select_vm:
                        my_logger.debug("VM Name:" + vm_name1 + "  Status:" + status)
                        if msg in "VM not found":
                            if operation in "Delete VM Info":
                                path="//div[@id='sanity_check_table']/table/tbody/tr["+str(row_count)+"]/td/a"  
                                driver.find_element_by_xpath(path).click()
                        ...
                        if "VM is on expected host" in msg :
                            ...
                            if public_ip!='Not Assigned':
                                ssh = paramiko.SSHClient()
                                ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
                                if ssh.connect(public_ip,username="root", password="duolc"):
                                    my_logger.debug("SSH True " + vm_name)
                                else:
                                    my_logger.debug("SSH False " + vm_name)
                                ssh.close()
                            break
...
def click_sanity(driver,xml_sub_child,my_logger):
    my_logger.debug("Inside click_sanity")
    isScroll(driver, xml_sub_child,my_logger)
    driver.find_element_by_link_text("Sanity Check").click()
    driver.find_element_by_xpath("//div/form/select/option[@value='0']").click()
    driver.find_element_by_xpath("//div/form/a/span[@class='icon-refresh']").click()
...
def check_host_state(driver,xml_child,xml_sub_child,my_logger):
    my_logger.debug("inside check_host_state***--")
    my_logger.debug("check_host_state")
    data=vm_list_all_vm(xml_child,xml_sub_child,driver,my_logger) 
    if data!=['', '','','']:
        vm_name=data[1][0:]
        host=data[2][0:]
    my_logger.debug("After vm_list_all_vm")
    isScroll(driver, xml_sub_child,my_logger)
    ...
    my_logger.debug("After clicking host_details")
    list_host(xml_child,xml_sub_child,driver,host,my_logger)
    my_logger.debug("Entering checkPendingTask")
    checkPendingTask(driver)
    my_logger.debug("Back to check host state")
    click_sanity(driver,xml_sub_child,my_logger)
    time.sleep(6)
    my_logger.debug("After Putting Host Down")
...
def main_host_check(driver,xml_child,xml_sub_child,my_logger):
    ...
    click_sanity(driver,xml_sub_child,my_logger)
    my_logger.debug("Before Putting Host Down")
...
def get_from_sanity(driver,xml_child,host_ip_given,my_logger):
    ...
    if isTablePresent(driver,xml_child,path_col,my_logger):
        ...
        for data in field:
            ...
            if (c_count%col_count==col_count-1):
                if select_host:
                    my_logger.debug("VM Name:" + vm_name + "  Status:" + status)
...
def put_a_host_down(driver,xml_child,xml_sub_child,my_logger):
    my_logger.debug("Inside put_a_host_down")
    host_ip_given=str(xml_sub_child.text)
    isScroll(driver, xml_sub_child,my_logger)
    driver.find_element_by_xpath("//a[@href='/baadal/admin/host_details']").click()
    time.sleep(5)
...
def create_many_vm(driver,xml_child,xml_sub_child,my_logger):
    my_logger.debug("Inside Create many vm")
    host_ip_given=str(xml_sub_child.text)
    my_logger.debug(host_ip_given)
    createflag=0
    migrateflag=0
    start=time.time()
    end=start+5400
    while start<end:
        ...
        data=list_my_vm(driver,xml_child,xml_sub_child,vm_name,my_logger)
        host_ip=data[1][0:]
        my_logger.debug("Host IP of newly created VM :" + host_ip)
        if host_ip=='':
            createflag=createflag+1
            my_logger.debug("VM creation unsucessful")
            if createflag==3:
                my_logger.debug("There is some problem in VM creation!Aborting VM creation")
                break
        elif host_ip_given!=host_ip:
            createflag=0
            #Migrate vm to the destination vm
            my_logger.debug("Migration Required")
            ...
            if host_ip_given!=new_host_ip:
                ...
                migrateflag=0
            if migrateflag==2:
                my_logger.debug("New VM's are unable to migrate to the destined host.Aborting VM creation")
                break
        else:
            my_logger.debug("VM is created in the destined host")
            createflag=0
        start=time.time()
...
def migrateVmTo(driver,xml_child,xml_sub_child,vm_name,my_logger):
    my_logger.debug("Inside migrateVmTo function")
    host_ip_given=str(xml_sub_child.text)
    driver.find_element_by_partial_link_text("All VMs").click()
    ...
    for option in el.find_elements_by_tag_name('option'):
        if option.text==host_ip_given:
            my_logger.debug("Option.text= " + option.text)
            option.click()
            time.sleep(5)
...
def pingf(driver,xml_child,xml_sub_child,vm_name,public_ip,my_logger):
	flag=0
        if public_ip=="Not Assigned":
	  my_logger.debug("VM is not pingable")
	else:
          my_logger.debug("Trial for VM pingable")
	mine = []
	check=os.system("ping -c4 "+public_ip)
	print check
	if check==0:
	   my_logger.debug("exist")
	   flag=1
	else:
	   my_logger.debug("not exist")
	for i in range(1, 2):
	    z =public_ip
...
def check_vm_configuration(driver,xml_child,xml_sub_child,vm_name,public_ip,my_logger):
    data2=[]
    my_logger.debug("inside check_vm_configuration")
    my_logger.debug(public_ip)
    memory_vm=execute_remote_cmd(public_ip,"root","grep MemTotal /proc/meminfo", "baadal",my_logger, ret_list = False)
    no_of_cpu=execute_remote_cmd(public_ip,"root","grep MemTotal /proc/meminfo", "baadal",my_logger, ret_list = False)    
    my_logger.debug("memory_vm : " + str(memory_vm))
    my_logger.debug("no of cpu : " + str(no_of_cpu))
...
def check_vm_in_pending_request_tablee(driver,xml_sub_child,xml_child,vm_name,my_logger):
    my_logger.debug("inside check_vm_in_pending_request_tablee")
...
def approval_reject_request(driver,xml_sub_child,xml_child,vm_name,my_logger):
        my_logger.debug("inside approval_reject_request ")
	driver.find_element_by_partial_link_text("All Pending Requests").click()
...
def edit_vm_conf(driver,xml_sub_child,xml_child,data,my_logger):
    vm_id=data[0][3:]
    vm_name=data[1][0:]
    public_ip=data[2][0:]
    status=data[3][0:]
    ...
    if status=="Running":
       ...
       driver.find_element_by_partial_link_text("My VMs").click() 	
       click_on_setting(driver,xml_sub_child,xml_child,vm_name,vm_id,my_logger)
       op_start_vm(driver,xml_sub_child,xml_child,vm_name,vm_id,my_logger)
       time.sleep(30) 
...
def check_file1(driver,xml_sub_child,ssh,public_ip,my_logger):
    ...
    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())     	 
    ssh.connect(public_ip,username="root", password="duolc")
    chan = ssh.get_transport().open_session()
    print "fhjshfhjsf"
    chan.exec_command("find /home/testmig/test.txt")
    return_status = chan.recv_exit_status()
	...
	print localstring
	if (return_status==0):		
	    my_logger.debug("vm Snapshot taken correctly")
	return 0
    else:
	my_logger.debug("not exist")
        return 1
...
def vnc_access(driver,xml_sub_child,xml_child,vm_name,baadal_db,my_logger):
	...
	if op=="check_vnc_session":
		check_vnc_access_time(driver,xml_sub_child,xml_child,baadal_db)
	elif op=="count_vnc_session":
		count_vnc_session(driver,xml_sub_child,xml_child,vm_name,baadal_db)
	return
...
def wake_on_lan(driver,xml_child,xml_sub_child,table_path,my_logger):
      my_logger.debug("inside wake on lan")
      if isElementPresent(driver,xml_child,table_path,my_logger):
         my_logger.debug("return back to checking table : ")
         ...
         if data!=['','','','']:
               ...
               owner_name=data[2][0:]
               host_name=data[3][0:]
               my_logger.debug("host name is : " + str(host_name))
               my_logger.debug("host name is : " + str(vm_id))
               op_name=xml_sub_child.get("op")    
               operation_name=op_list[op_name]
               my_logger.debug("operation_name is "+ str(operation_name))
               if vm_id !="":
                   click_on_setting(driver,xml_sub_child,xml_child,vm_name,vm_id,my_logger)
...
def shut_down_host(driver,xml_sub_child,xml_child,host_first,vm_name,operation_name,my_logger):
    ...
    if isTablePresent(driver,xml_child,path_col,my_logger):
        ...
        for hdata in header_field: 
            ...
            count+=1
        my_logger.debug("host_num is : " + str(host_num))
        my_logger.debug("host_cmd is : " + str(host_cmd))
        my_logger.debug("host_status is : " + str(host_status_no))
        col_count=count
        field=driver.find_elements_by_xpath(path_col)
        for data in field:
            ...
            if c_count%col_count==host_cmd:
                ...
                my_logger.debug("host cm is : " + str(host_cm))
            if c_count%col_count==host_status_no:
                host_status=data.text
                my_logger.debug("host status is : " + str(host_status))           
            if (c_count%col_count==col_count-1):
                  my_logger.debug("host is : " + str(host))
                  my_logger.debug("host first is : " + str(host_first))
                  row_count+=1
                  my_logger.debug("row_count is : " + str(row_count))
                  if (host in host_first) & (host_status=="Up"):
                     my_logger.debug("inside host checking if block  " + str(host_cm))
                     driver.find_element_by_xpath('//table/tbody/tr/td[@id="%s"]/a'%host_cm).click()
                     my_logger.debug("sleep for 60 seconds")
                     time.sleep(30)
                     my_logger.debug("shutting down the host")
                     click_on_dialogbox(driver)
                     flag=check_task_task_report(driver,xml_sub_child,xml_child,vm_name,operation_name,my_logger)
...
def check_migration_result(driver,xml_sub_child,xml_child,vm_name,vm_id,host_first,host_second,my_logger):
    my_logger.debug(" inside check_migration_result")
    path_row="//table[@id='listallvm']/tbody/tr"
    path_col="//table[@id='listallvm']/tbody/tr/td"
    path_header="//table[@id='listallvm']/thead/tr/th"
    if isTablePresent(driver,xml_child,path_col,my_logger):
        ...
        for data in field:
            ...
            if (c_count%col_count==col_count-1):
                  if (vm_name1==vm_name) & (host in host_second):
                      my_logger.debug("vm_name:" +str(vm_name)+ "host:"+str(host_second))
                      return True
            c_count+=1 
        return False
...
def migrate_to_host(driver,xml_sub_child,xml_child,vm_name,vm_id,my_logger):
    my_logger.debug("inside_migrate_operation")
    op_name=xml_sub_child.get("op")
    limit=0
    op_name=xml_sub_child.get("op")
    operation_name=op_list[op_name]
	...
	driver.find_element_by_partial_link_text("All VMs").click()
        vm_user_list=total_user(driver,xml_sub_child,xml_child,vm_name,vm_id,my_logger)
        my_logger.debug("click_migrate_button")
	driver.find_element_by_xpath("//a[@title='Migrate this virtual machine']").click()
	if xml_sub_child.get("live")=="yes":
           message=message_flash(driver,xml_sub_child,xml_child,my_logger)
           message=str(message)
           if message=="No host available right now":
                my_logger.debug("No host available right now. Please start/create the new host")
                sys.exit()
           else:
