...
db_name="/baadal/baadal_nat.db"
table=mappings

if test $# -eq 0; then

	ip_mapping_list=(`echo "$(sqlite3 baadal_nat.db 'select public_ip,private_ip from mappings;')" | tr " " " "`)
	...
	for ip_mapping in ${mapping_list[@]}; do
		...
		ifconfig eth0:$private_ip $public_ip up	2>> restart.err	
	...
	if test $1 == 'add' || test $1 == 'remove'; then
		...
		if test "$2" != "$3" && test ${public_ip[0]} -ne 0 && test ${public_ip[0]} -le 255 && test ${public_ip[1]} -le 255 && test ${public_ip[2]} -le 255 && test ${public_ip[3]} -le 255 && test ${public_ip[3]} -ne 0 && test ${private_ip[0]} -ne 0 && test ${private_ip[0]} -le 255 && test ${private_ip[1]} -le 255 && test ${private_ip[2]} -le 255 && test ${private_ip[3]} -le 255 && test ${private_ip[3]} -ne 0; then
			...
			if test -f $db_name && test "$(sqlite3 baadal_nat.db '.schema $table')" != ""; then
		        ...
		        if test "$(cat iptables.err)" == "";

			        ifconfig eth0:$3 $2 up 2> ipaliasing.err
			        
							if test "$(cat ipaliasing.err)" != "";
