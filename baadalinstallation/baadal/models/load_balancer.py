...
#Function perform loadbalancing of system based on utilization data and shutdown unused hosts
...
def find_host_and_guest_list():
    ...
    hosts = db(db.host.id>0 and db.host.status == HOST_STATUS_UP).select(db.host.ALL)
    for host in hosts:
