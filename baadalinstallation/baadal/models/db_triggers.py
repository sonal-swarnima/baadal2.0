...
def schedule_task(fields, _id):
    ...
    if fields['task_type'] == TASK_TYPE_CLONE_VM:
        ...
        for clone_vm_id in fields['parameters']['clone_vm_id']:
            vm_scheduler.queue_task('clone_task', pvars = dict(task_event_id = task_event_id, vm_id = clone_vm_id),start_time = request.now, timeout = 1800)
    else:
        vm_scheduler.queue_task('vm_task', pvars = dict(task_event_id = task_event_id),start_time = request.now, timeout = 1800)
